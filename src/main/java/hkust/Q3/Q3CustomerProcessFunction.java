...
public class Q3CustomerProcessFunction extends KeyedProcessFunction<Object, Node, Node> {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        ValueStateDescriptor<Set<Node>> descriptor = new ValueStateDescriptor<>("customer alive", setDescriptor);
        alive = getRuntimeContext().getState(descriptor);

    }

    public boolean isValue(Node node){
        if (node.getValueByName("C_MKTSEGMENT").equals("BUILDING")){
            return true;
        }else{
            return false;
        }
