...
public class Q3LineitemProcessFunction extends KeyedCoProcessFunction<Object, Node, Node, Node> {
    ...
    public void open(Configuration parameters) throws Exception {
        ValueStateDescriptor<Integer> count_descriptor = new ValueStateDescriptor<Integer>("lineitem alive number",Integer.class);
        ...
        ValueStateDescriptor<Set<Node>> set_descriptor = new ValueStateDescriptor<>("lineitem tmp node", tmp_node_descriptor);
        tmp_node = getRuntimeContext().getState(set_descriptor);

        ValueStateDescriptor<Node> alive_node_descriptor = new ValueStateDescriptor<Node>("lineitem node", Node.class);
    ...
    public boolean isValid(Node node) throws ParseException {
        ...
        if (((Date) node.getValueByName("L_SHIPDATE")).compareTo(format.parse("1993-03-30"))>0 ){
            return true;
        }
        else {
            return false;
